...
interface IObject {
  [prop: string]: { cvalue: number | string | IObject | undefined } | undefined
}

function summ(a: IObject) {
  const x = Object.keys(a).map(k => {
    const elem = a[k]

    if (typeof elem === "undefined") return 0
    if (typeof elem.cvalue === "undefined") return 2022
    if (typeof elem.cvalue === "string") return elem.cvalue === "0" ? 0 : +elem.cvalue || 2022
